...
import filereaders.individual.DealedVariation.DelMap2SubElement;
import filereaders.individual.VariantAnalysis.ControlArea;
import filereaders.individual.vcf.Variant;
import filereaders.tools.LinkedArrayList;
import filereaders.tools.LinkedArrayList.Entry;


import static filereaders.Consts.*;
import static filereaders.individual.EctypalSubElement.*;
import static filereaders.individual.GeneticCode.*;
import static filereaders.individual.vcf.Variant.*;
...
public class EctypalElement {
	...
	private Entry<EctypalSubElement> dealAINS(Entry<EctypalSubElement> cur, Variant v, Integer curNeedToDealType) throws IOException{
		...
		if(v.getLetter().length() % 3 == 0){
			...
			if(remain != 3)
				...
				if(trans.indexOf("$")<0 && after.indexOf("$")>=0 && v.getEffect()<7)//terminator gain
	...
	private Entry<EctypalSubElement> dealADEL(DelMap2SubElement dms) throws IOException{
		...
		if(dms.boxBases % 3 == 0){
			...
			if(remain != 1)
				...
				if(after.indexOf("$")<0 && pre.indexOf("$")>=0 && del.getEffect()<6)//terminator loss
	...
	private MultiFromVariant recordVariant(Extract3Bases e3b, Variant v, String transcription, EctypalSubElement cur){
